...
<title>My Donations - <?php echo $siteTitle; ?></title>
...
<?php
	//get donations
	$result = mysqli_query($conn, "SELECT * FROM donations where donor_id = '" .$dnnUsers["id"]. "' order by id desc");
?>
...
<tr>
		...
		?>
        	<i class="ion-android-cancel" style="font-size:36px; color:#777;"></i><br />Cancel Donation
        <?php
			}
		...
		?>
			<a href="setCancelDonation.php?q=<?php echo $dnnUsers["id"]; ?>" title="Approve"><i class="ion-android-cancel" style="font-size:36px; color:#f00;"></i><br />Cancel Donation</a>
		<?php
			}
		?>
        </td>
