		...
		if($_POST['password']==$_POST['passverif'])
		{
			...
			if(strlen($_POST['password'])>=6)
			{
				...
				if(preg_match('#^(([a-z0-9!\#$%&\\\'*+/=?^_`{|}~-]+\.?)*[a-z0-9!\#$%&\\\'*+/=?^_`{|}~-]+)@(([a-z0-9-_]+\.?)*[a-z0-9-_]+)\.[a-z]{2,}$#i',$_POST['email']))
				{
					...
					$dn = mysqli_fetch_array(mysqli_query($conn, 'select count(*) as nb from members where id="'.$idP.'"'));

						//We edit the user informations
						if(mysqli_query($conn, 'update members set password="'.$password.'", email="'.$email.'", phone="'.$phone.'", name="'.$name.'", state="'.$state.'", city="'.$city.'" where id="'.$idP.'"'))
						{
							//We dont display the form
							$form = true;     
?>
<div class="alert alert-success" role="alert" style="text-align:center;">Account Details was successfully update!
<br />
<a href="profile.php?id=<?php echo $idP; ?>">Click here</a>
</div>
<?php
						}
						else
	...
	if($form)
	{
		...
		else
		{
			...
			$dnn = mysqli_fetch_array(mysqli_query($conn, 'select * from members where id="'.$idP.'"'));
...
<form action="profileEdit.php?id=<?php echo $idP; ?>" method="post" class="form">
